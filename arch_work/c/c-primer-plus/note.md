...
- 使用该头文件后可以与c++兼容，c++使用`bool,false,true`作为关键字
...
### 6.10 c语言不会检查数组的下标是否正确，下标从0开始计数其他的元素依次排列
...
```
### 7.2 getchar(), putchar(), 在stdin.h 中的预处理宏函数

- 字符输入/输出函数,他们只处理字符, 所以相对于scanf和printf 更快，更简洁，而且他们不需要转换说明
