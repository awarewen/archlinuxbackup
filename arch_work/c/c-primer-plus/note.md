...
fgets 第二个参数是限定输入字符的数量n,而实际能输入的字符数为n-1, 留出一位是放\0空字符
并且读取\n换行符将其储存在字符串中
...
* strncat(words1, words2, 5);  
  ...
  与strncat用法差不多，多了第三个参数，用来限制附加到第一个字符串后面的字符数
...
1. 字符串类型数值转换为int类型数值
  ...
  如果参数是字符字符串则返回0
  chars = "abc";
  atoi (chars);   //返回0,c标准规定这是未定义的行为

  如果是以数值开头的字符串则返回开头的整数
  atoi("42chars"); // 返回42
