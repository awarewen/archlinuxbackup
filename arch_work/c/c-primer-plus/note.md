...
*(pr + 1); //3
```
### 9.17 在处理一个基本类型时如何选择在函数中传参时选择指针还是实参
...
1. `fgets`  
    ...
    - `fgets` 第二个参数是限定输入字符的数量`n`,而实际能读取的字符数为`n-1`, 留出一位是放`\0`空字符  
    - 并且读取`\n`换行符将其储存在字符串中  
