...
### 6.1 while & for
- while
    - while 循环条件仅有一个参数列表,while适合完成简单的循环任务  
    - `while(测试参数列表) {语句块}`  
...
- expression 的值为非0时while进入第一次循环,当值为0跳出循环,或者使用break跳出循环   
...
```  
- 基本的for循环参数列表`(变量初始化; 循环条件检测; 变量更新)` for括号包含了三个参数列表，他们都是*完整表达式*  
...
### 6.5 _____ __Bool变量 和 stdbool.h

- `_Bool input;` //变量input只储存0和1,代表假和真，其他任何的非零值都会变成1
- c99协议提供了`stdbool.h`头文件用`bool` 代替`_Bool`, 并分别用`false,true`分别定义为了`0, 1`
