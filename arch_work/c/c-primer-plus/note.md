...
一般出现这种情况是磁盘已满，移动硬盘被移除或者出现i/o错误，都会导致fclose调用失败
...
int setpos (FILE *stream, const fpos_t *pos);
...
标准i/o使用
 调用fopen()打开文件
1. c程序会创建一个缓冲区(读写模式写创建两个缓冲区)以及一个包含文件和缓冲区的结构
...
2. 调用一个定义在stdio.h中的输入函数
 ...
 在填充缓冲区的后将fp指向结构中的值，设置流中当前的位置和拷贝进缓冲区的字节数，通常当前位置从字节0开始
...
3. 当输入函数读取完缓冲区的所有字符时，会请求将下一个缓冲大小的数据块填充到缓冲区中，以这种方法直到读取完整个文件
 ...
 输出函数以类似的方式将数据写入缓冲区，当缓冲区被填满，数据将被拷贝到文件中


## 其他的一些i/o函数
### int ungetc (int c, FILE *******fp)
 int ungetc ()函数将c指定的字符放回输入流中
 下一次调用标准输入时将读取到该字符
...
### int setvbuf (FILE ******* restrict fp, char * restrict buf, int mode, size_t size);
  setbuf()函数创建一个提供标准i/o函数替换使用的缓冲区
