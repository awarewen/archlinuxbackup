...
指针赋值和const需要注意的规则
除了可以两个const 元素互相赋值外，还可以将非const 数据赋值给const 元素
doule rates[5] = {0};
...
* 关于指针赋值和const需要注意的规则，将const 或非const数据的地址初始化为指向const的指针或为其赋值是合法的
...
#define CHS "abcd"
printf("%s\n", CHS + 1); // 结果为bcd , 字符串指针指向该字符串的下一个地址
...
## 读取字符串输入
* c99以上的版本不建议再使用gets 和 puts 函数
gets　在输入时不会检查输入的字符串是否溢出

* fgets
c11中fgets()函数可以作为gets的替代
fgets 第二个参数是输入字符的数量n-1留出一位是放\0空字符
并且读取\n换行符将其储存在字符串中
第三个参数指明从何处读取输入，键盘输入为 stdin
fgets(chars, n-1, stdin);
...
### strcat()和strncat()将两个字符串拼接返回第一个字符串首地址
  ...
  strncat(words1, words2, 5); //前提是words1能够存放两个字符串所有内容并且能储存\0结束符
...
### strcmp 和 strncmp 将用户响应的字符串和以储存的字符串作比较 (不是比较单个字符)
  * 比较两个字符串第一个不同的字符根据大小返回相应的返回值
  * strcmp 和strncmp 根据字符的机器排序决定排序大小
  strcmp (input, "abc");
